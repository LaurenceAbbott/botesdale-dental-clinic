...
**The column heading is a title, not a link.** It used to be both — the category name doubled as the link to its overview — and a heading that is also a link leaves the reader guessing which it is. The overview page is now the first item in the list instead, named in full (*Cosmetic dentistry overview*), so everything clickable in the panel looks and behaves the same, and **Missing teeth** stops being a heading over nothing. The titles drop the trailing *dentistry*: three of the four carried it, so it was the least informative word on the row. Every item takes a chevron that steps out on hover, in a fixed gutter so labels do not shift.
