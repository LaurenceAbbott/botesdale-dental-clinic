...
4. **One accent, sparingly.** `--accent` appears in the arc motif, list markers, focus rings, validation, text links (§4.2) and the fill of the one primary button a page is allowed (§4.1). It is never used as a background for large areas — a button is the largest thing it fills. **The cookie banner's Accept is the one exception to "one per page"**: the banner is an overlay a visitor has to clear before reading anything, not part of the page's own hierarchy, so it carries its own primary and does not spend the page's. `verify-cta.js` scopes the one-primary count with `:not(.cookie)` for exactly this reason. On the banner's dark ground the accent fill reads 5.8:1 and its ink label 5.8:1 on the fill.
...
**The placeholder is rounded too**, because a standalone `.ph` — `.figure` in prose, `.home-intro__media`, the `.ar-*` demos — *is* the image object and nothing above it clips. Where it fills a container that already clips (`.ed__media`, `.shot`, `.team__portrait`, `.strip__media`, `.card__media`) the radius is zeroed on it, so the container owns the corner and no rounded box appears inside a square one.

`verify-radius.js` walks up from every image and placeholder until it finds something clipping at 32px, and fails if nothing does — 119 of them per width. It checks the *chain*, not the image, so it stays correct however the radius is distributed between tile, row and card. The nav/footer brand marks and the arc-clipped hero panels are the only exclusions.
...
Inside `.prose` the arc centres on the **measure**, not the container: the paragraphs are capped at 68ch — 567px inside a 900px `.wrap--narrow` — so an arc centred on the container sat 118px right of the text and read as misaligned. Dropping the cap instead would put the line at roughly 108 characters, well past readable, so the arc moves rather than the measure.
...
**Crowns and bridges live under Missing teeth**, not Cosmetic. Their own copy decides it: a bridge is *"a fixed replacement anchored to the teeth beside the gap"*, and Missing teeth's own summary already names both. Crowns is the arguable one — its page is about rebuilding a broken tooth rather than replacing an absent one — but a crown is also what finishes an implant, and one clear home beats a page filed under two. `GROUPS`, the leaf `parent`/`group`, the category strips and the nav all move together, because the nav is built from `GROUPS`.

**The column heading is a title, not a link.** It used to be both — the category name doubled as the link to its overview — and a heading that is also a link leaves the reader guessing which it is. The overview page is now the first item in the list instead, named in full (*Cosmetic dentistry overview*), so everything clickable in the panel looks and behaves the same, and **Missing teeth** stops being a heading over nothing. The heading takes the `.statement-text` treatment at a small size — head font, semibold, ink, sentence case — which is what `.menu__subhead` uses in the sheet, so both menus name a section the same way. It briefly ran as an uppercase micro-label with the category word shortened (*General*); that read as a tag rather than the name of the section, and as a different thing from the page it heads. Every item takes a chevron that steps out on hover, in a fixed gutter so labels do not shift.

**It is a card, not a band.** Full-bleed and ruled between the columns, it read as a table dropped over the page. Inset to 1180px, centred, on `--paper-2` with one hairline and the `--radius-media` corner, it reads as a surface belonging to the menu. The columns and the footer are one element — styled separately they rendered as two cards with a gap. Each column carries the category's own one-line description, which is also what stops **Missing teeth**, the category with no sub-pages, reading as an empty column. Item hover is a small rounded pill rather than a full-width band.

**The sheet is a tree, and its indents say so.** Headings and items used to sit at the same left edge, so three levels read as one flat list. The category heading now starts on the same line as the group title above it — *Treatments* — and everything clickable sits one step in. It is a `<span>`, not an anchor, so it cannot be tapped; its overview page is the first item beneath it. The sheet uses the **full** category name (*General dentistry*) while the desktop column uses a short title (*General*), because a column four words wide has a blurb underneath to say the rest and a tree row does not.

**The pointer has to be able to reach it.** The panel hangs off the header's bottom edge, not the link's, so there is a band of dead space between them; on `:hover` alone the menu closed the instant the pointer entered it, and a diagonal move toward a far column crosses it every time. `nav.js` holds the panel open for 260ms after the pointer leaves and cancels that on re-entry. `:hover` and `:focus-within` stay in the stylesheet, so without JS it degrades to the old behaviour rather than breaking. `verify-navhover.js` walks the pointer from the link, through the gap, diagonally into the last column, and clicks a link — removing the delay fails it at both widths.
